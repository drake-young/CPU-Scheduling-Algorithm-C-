...
//
//*********************************************************
bool SJFQueue::enqueue(dataType dataValue) {
	...
	if (headPtr == nullptr) {
		...
		Node *currentPtr = headPtr;
		while (currentPtr->getNext() != nullptr) {
			if (dataValue.getCPUBurstTime() < currentPtr->getNext()->getValue().getRemainingTime())
			{
				break;
			}
			currentPtr = currentPtr->getNext();
